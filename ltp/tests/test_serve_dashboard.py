...
class DashboardServerTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.project / "ltp" / "generated").mkdir(parents=True)
        (self.project / "ltp" / "generated" / "dashboard-model.json").write_text(
            '{"project": {"name": "Test"}, "entities": [], "views": {}}',
            encoding="utf-8",
        )
    ...
    def test_serves_only_dashboard_and_known_model(self) -> None:
        ...
        self.assertIn("application", headers["Content-Type"])

    def test_rejects_traversal_and_unknown_paths(self) -> None:
        for path in (
            "/assets/%2e%2e/index.html",
            "/assets/../index.html",
            "/project/ltp/generated/dashboard-model.json",
            "/api/model",
        ):
    ...
    def test_is_read_only(self) -> None:
        status, _, body = self.request("POST", "/api/dashboard")
        self.assertEqual(status, 405)
        self.assertIn(b"read only", body)

    def test_meta_reports_the_dashboard_model(self) -> None:
        status, _, body = self.request("GET", "/api/meta")
        self.assertEqual(status, 200)
        self.assertIn(b'"exists":true', body)
        self.assertIn(b"dashboard-model.json", body)
